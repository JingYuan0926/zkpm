  ...
  return (
    <div className="bg-[var(--card)] border border-gray-200 rounded-xl p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-muted/50 border border-gray-200 rounded-lg p-4">
          <div className="flex items-center gap-2 text-gray-600 text-sm mb-2">
            <Activity className="w-4 h-4" />
            <span>Liquidity Parameter (b)</span>
          </div>
          <p className="text-2xl font-semibold">{liquidityParameter.toFixed(2)}</p>
          <p className="text-xs text-gray-600 mt-1">LS-LMSR depth</p>
        </div>
        ...
        <div className="bg-muted/50 border border-gray-200 rounded-lg p-4">
          ...
          <p className="text-xs text-gray-600 mt-1">b = α × Volume</p>
        </div>
      </div>
      ...
      <div className={`p-4 rounded-lg border mb-6 ${
        ...
        <div className="flex items-start gap-3">
          <AlertTriangle className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
            volatilitySpread.active ? 'text-yellow-400' : 'text-gray-600'
          }`} />
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              <h4 className={`text-sm font-semibold ${
                volatilitySpread.active ? 'text-yellow-400' : 'text-gray-800'
              }`}>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <p className="text-gray-600 text-xs">Base Spread</p>
                <p className="font-medium">{(volatilitySpread.baseSpread * 100).toFixed(1)}%</p>
              </div>
              <div>
                <p className="text-gray-600 text-xs">Current Spread</p>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          {inventorySkew.map((item, idx) => {
            ...
            return (
              <div key={idx} className="bg-muted/50 border border-gray-200 rounded-lg p-3">
                <div className="flex items-center justify-between mb-2">
      ...
      <div className="bg-muted/50 border border-gray-200 rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-sm font-medium text-gray-800">LP Vault Utilization</h4>
